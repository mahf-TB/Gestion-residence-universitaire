  ...
  <div class="absolute -top-[150px] w-full text-white   rounded-md text-center  text-xl ">
    <div class="flex items-center justify-around">
      ...
      <div class="px-4 py-3 bg-white text-red-600 rounded shadow cursor-pointer">
        <h1 class="text-lg flex items-center justify-between">Demande non valider
          <span class="ml-5 bg-red-100 px-3 py-2 rounded"><i class="fa-solid fa-user-xmark"></i></span>
        </h1>
        <div class="text-3xl flex items-center justify-between mt-3 text-blue-3">
      ...
      <div class="px-4 py-3 bg-white text-green-600 rounded shadow cursor-pointer">
        <h1 class="text-lg flex items-center justify-between">Demande valider
          <span class="ml-5 bg-green-100 px-3 py-2 rounded"><i class="fa-solid fa-user-check"></i></span>
        </h1>
        <div class="text-3xl flex items-center justify-between mt-3 text-blue-3">
          <span>{{ count.accepte }}</span>
          <span class="text-sm ml-3 cursor-pointer flex items-center hover:text-blue-1"
            @click="functStatus('accepter')">
            Cliquez ici pour afficher
            <i class="fa-solid fa-angle-right ml-1"></i> </span>
        </div>
      </div>
...
export default {
  ...
  methods: {
    async getNbDemande() {
      try {
        var response = await Axios.get('/reservation_status')

        let data = response.data;
        data.forEach(items => {
          if (items.status == 'accepter') {
            this.count.accepte = items.nombre
          } else if (items.status == 'refuser') {
